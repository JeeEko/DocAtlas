...
import { getRequiredDocPaths, getEnterpriseTemplateDirs } from './taxonomy.js';
...
export function bootstrapProject(projectRoot, options = {}) {
  ...
  writeDocAtlasJson(projectRoot, { journeyName, force, toolkitRoot, taxonomy });
  ...
  copy(path.join(templatesDir, 'docs/AI_CONTEXT.md'), path.join(docAtlasRoot, 'docs/AI_CONTEXT.md'));
  ...
  const agentsDir = path.join(templatesDir, '.cursor/agents');
  if (fs.existsSync(agentsDir)) {
    for (const name of fs.readdirSync(agentsDir)) {
      if (!name.endsWith('.md')) continue;
      copy(path.join(agentsDir, name), path.join(projectRoot, '.cursor/agents', name));
    }
  }

  if (taxonomy === 'enterprise') {
    for (const dir of getEnterpriseTemplateDirs()) {
      copyDir(
        path.join(templatesDir, 'docs', dir),
        path.join(docAtlasRoot, 'docs', dir),
        { force }
      );
    }
  }
  ...
  copy(
    path.join(templatesDir, '.github/workflows/docatlas-update-report.yml'),
    path.join(projectRoot, '.github/workflows/docatlas-update-report.yml')
  );
...
function writeDocAtlasJson(projectRoot, { journeyName, force, toolkitRoot, taxonomy = 'standard' }) {
    ...
    `${JSON.stringify(
      {
        ...
        draftedAt: null,
        docTaxonomy: taxonomy,
        lastAnalyzedCommit: null,
        lastAnalyzedAt: null,
        lastAnalyzedBranch: null,
        includePaths: [],
        excludePaths: [],
        agentRules: [],
        analyzedVersions: [],
