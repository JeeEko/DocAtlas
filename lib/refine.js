...
function writeJourneyDoc(layout, journeyName, steps, scan, mode) {
  ...
  const modeNote =
    mode === 'discovery'
      ? 'DocAtlas scanned routes and pages across the repo (not just one login flow).'
      : `Traced using ${mode} flow patterns from code.`;
...
Auto-scanned by \`docatlas refine\` (${mode} mode). Not exhaustive — verify against code.
