...
export function runRefine(projectRoot, options = {}) {
  const meta = readJson(path.join(projectRoot, '.docatlas.json'));
  ...
  const journeyRel = `docs/JOURNEY-${slug(journeyName)}.md`;
  writeJourneyDoc(projectRoot, journeyRel, journeyName, steps, scan);
  updateArchitecture(projectRoot, journeyName, steps, apiRoutes, webPages, scan);
  updateGlossary(projectRoot, journeyName, terms);
  updateStartHere(projectRoot, journeyName, journeyRel, steps);
  touchMeta(projectRoot, journeyName);

  return { journeyRel, steps: steps.length, terms: terms.length };
}

function slug(name) {
  return name.replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-|-$/g, '') || 'main';
}
...
function buildJourneySteps(webPages, apiRoutes, clientApi, journeyName) {
  ...
  for (const flow of FLOW_STEPS) {
    ...
    const relatedRoutes = apiRoutes.filter((r) =>
      flow.keywords.some((k) => r.path.toLowerCase().includes(k))
    );
...
function stepMatchesClient(c, flow) {
  if (flow.id === 'catalog') {
    return ['getProducts', 'getProduct'].includes(c.fn) || /\/api\/products/.test(c.path);
  }
  if (flow.id === 'cart') {
...
function writeJourneyDoc(projectRoot, journeyRel, journeyName, steps, scan) {
...
`;
  ...
  writeText(path.join(projectRoot, journeyRel), content);
}

function updateArchitecture(projectRoot, journeyName, steps, apiRoutes, webPages, scan) {
...
function updateGlossary(projectRoot, journeyName, terms) {
  ...
  writeText(
    path.join(projectRoot, 'docs/GLOSSARY.md'),
    `# Glossary
...
function updateStartHere(projectRoot, journeyName, journeyRel, steps) {
  const startPath = path.join(projectRoot, 'docs/START_HERE.md');
...
function touchMeta(projectRoot, journeyName) {
  const metaPath = path.join(projectRoot, '.docatlas.json');
