...
import { linkContextsToJourneys } from './cross-ref.js';
...
export function runRefine(projectRoot, options = {}) {
  ...
  touchMeta(layout, journeyName, mode, steps.length, coverage, frameworks, apiRoutes);
  linkContextsToJourneys(projectRoot, journeyName, apiRoutes);
  seedHybridArchitectureDocs(layout, scan, inventory, apiRoutes, frameworks);
...
function touchMeta(layout, journeyName, mode, stepCount, coverage, frameworks, apiRoutes) {
  const meta = readJson(layout.metaAbs) ?? {};
  ...
  meta.routeInventory = apiRoutes.map((r) => r.path);
...
function seedHybridArchitectureDocs(layout, scan, inventory, apiRoutes, frameworks) {
  const integrationsDir = path.join(layout.docsDirAbs, 'integrations');
  const systemDir = path.join(layout.docsDirAbs, 'system');
  const apiPath = path.join(integrationsDir, 'internal-apis.md');
  const mapPath = path.join(systemDir, 'component-map.md');

  if (apiRoutes.length && !fs.existsSync(apiPath)) {
    const lines = apiRoutes.slice(0, 40).map((r) => `- \`${r.path}\` — ${r.method ?? 'GET'} [likely] (${r.source ?? 'parser'})`);
    writeText(
      apiPath,
      `# Internal APIs

> Auto-drafted by \`docatlas refine\`. Validate during discovery.
...
See route inventory in [ARCHITECTURE.md](../ARCHITECTURE.md).
`
    );
  }

  if (scan.packages.length && !fs.existsSync(mapPath)) {
    const rows = scan.packages.map((p) => {
      const rel = path.relative(layout.projectRoot, p.dir).replace(/\\/g, '/') || '.';
      return `| ${p.name} | \`${rel}/\` | ${p.pkg?.description ?? 'module'} [likely] |`;
    });
    writeText(
      mapPath,
      `# Component map

> Auto-drafted from package layout. Validate during discovery.
