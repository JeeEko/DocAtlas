...
function suggestJourney(routes, packages) {
  ...
  if (text.includes('checkout')) return 'MockCheckout';
  if (text.includes('login') || text.includes('auth')) return 'UserLogin';
  if (text.includes('cart')) return 'ShoppingCart';
  if (text.includes('order')) return 'PlaceOrder';
  if (packages.length > 1) return packages[0]?.name?.split('/').pop() ?? 'MainFlow';
  return 'MainFlow';
}
