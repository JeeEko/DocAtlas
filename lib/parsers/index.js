...
export function collectApiRoutes(projectRoot, packages, frameworks) {
  ...
  for (const dir of dirs) {
    walkFiles(dir, (file) => {
      ...
      if (frameworks?.trpc) routes.push(...parseTrpc(content, relFile));
    });
...
function parseTrpc(content, file) {
  const routes = [];
  const procRe = /(\w+)\s*:\s*(?:publicProcedure|protectedProcedure|adminProcedure|procedure)/g;
  let m;
  ...
  const routerRe = /createRouter\s*\(\s*\{([\s\S]*?)\}\s*\)/g;
  let rm;
  while ((rm = routerRe.exec(content))) {
    const block = rm[1];
    const nameRe = /(\w+)\s*:/g;
    let nm;
    while ((nm = nameRe.exec(block))) {
      if (['create', 'merge', 'router'].includes(nm[1])) continue;
      routes.push({
        method: 'RPC',
        path: `/trpc/${nm[1]}`,
        file,
