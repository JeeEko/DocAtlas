...
export async function runInit(projectRoot, options) {
  ...
  if (isDocAtlasInitialized(projectRoot) && !force) {
    throw new Error('DocAtlas already set up. Use --force to re-run, or delete doc-atlas/.docatlas.json.');
  }
  ...
  bootstrapProject(projectRoot, { force: true, withGovernance, journeyName, taxonomy });
  ...
  console.log('  Done!');
  console.log('');
  console.log(`  Open:     ${layout.docRel('START_HERE.md')}`);
  console.log('  Loop:     init → refine → discovery → drift');
  console.log('  Next:     /docatlas  (full journey in Cursor)');
  console.log('  Or CLI:   docatlas refine');
  console.log('  Cursor:   /docatlas — runs all phases via workflow skills');
  console.log('');
  console.log('  (Install DocAtlas once: npm install -g github:JeeEko/DocAtlas)');
  console.log('');
...
function patchAgents(projectRoot, journeyName) {
  const layout = getLayout(projectRoot);
  const agentsPath = layout.agentsAbs;
