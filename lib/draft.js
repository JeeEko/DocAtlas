...
function writeArchitecture(projectRoot, scan, name) {
  ...
  const routeLines = scan.routes.slice(0, 12).map((r) => `- \`${r.path}\` (${r.type}) [likely]`);
...
Primary journey suggested: **${scan.journey}**. Trace UI pages → API handlers → storage in a follow-up pass (Cursor skill or \`docatlas refine\`).
...
function updateDocAtlasJson(projectRoot, journeyName) {
  const filePath = path.join(projectRoot, '.docatlas.json');
