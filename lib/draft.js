...
export function draftDocumentation(projectRoot, scan, journeyName) {
  ...
  files.push(writeBusiness(projectRoot, scan, name));
...
function writeBusiness(projectRoot, scan, name) {
  const layout = getLayout(projectRoot);
  const rel = layout.docRel('BUSINESS.md');
  const capabilityLines = scan.packages
    .map((p) => {
      const desc = p.pkg?.description ?? inferPackageRole(p.name, p.pkg);
      return `- **${p.name}** — ${desc} [likely]`;
    })
    .join('\n');
...
What this product does in business terms — the *why* and *who*, not file paths.

> Auto-drafted from README and package metadata. **Discovery must confirm** rules and actors against code, tests, or product docs.

## What problem does this solve? [likely]
...
| Actor | Goal |
|-------|------|
| TODO | Fill during discovery — infer from README, roles in code, or ask stakeholder |

## Core capabilities [likely]

Inferred from repo packages and layout:

${capabilityLines || '- TODO: list what the product enables for users'}

## Business rules [uncertain]

DocAtlas cannot infer rules reliably from code alone. During **discovery**, extract from:

- README / product docs
- Validation logic and tests
- Domain models and error messages
- Comments and ADRs
...
- TODO: what this project explicitly does not handle

## Related docs

- [Architecture](ARCHITECTURE.md) — technical design
- [Journey](JOURNEY-*.md) — flows traced in code (after refine)
...
3. In Cursor: \`/docatlas-refine\` → \`/docatlas-discovery\` → \`/docatlas-drift\`
...
3. **\`/docatlas-discovery\`** — explore codebase and fill in docs (**required**)
...
| File | Why |
|------|-----|
| [BUSINESS.md](BUSINESS.md) | **Why** the product exists — problem, users, rules |
